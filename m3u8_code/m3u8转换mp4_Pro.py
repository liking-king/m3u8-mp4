...
class M3U8Downloader:
    def __init__(self, m3u8_urls=None, output_file="output.mp4", max_workers=10):
        self.m3u8_urls = m3u8_urls or []
    ...
    def start_download(self):
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            futures = {executor.submit(self._download_segment, seg, i, base_url): i for i, (seg, base_url) in enumerate(segments_with_url)}
    ...
    def create_gui(self):
        ...
        Label(self.window, text="M3U8 URLs (每行一个):").grid(row=0, column=0, padx=10, pady=5)
        self.m3u8_urls_text = Text(self.window, height=5, width=50)
        self.m3u8_urls_text.grid(row=0, column=1, padx=10, pady=5)
    ...
    def start_download_thread(self):
        ...
        if not self.m3u8_urls or not self.output_file:
            messagebox.showwarning("输入错误", "请确保至少输入一个M3U8 URL并选择保存路径。")
            return
